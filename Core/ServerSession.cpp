...
{
    // CMemoryPool g_overlapPool을 사용하여 IOContext를 할당한다고 가정합니다.
    ...
    catch (const std::bad_alloc& e)
    {
        ...
        std::cerr << "Fatal Error: Failed to allocate IOContext. " << e.what() << std::endl;
...
void ServerSession::Disconnect()
{
    if (m_sock != INVALID_SOCKET)
    {
        ...
        closesocket(m_sock);
        m_sock = INVALID_SOCKET;
        // std::cout << "ServerSession Disconnected." << std::endl; // 로그 예시
    }
